...
for required in README.md RELEASE_HARDENING.md "RELEASE_NOTES_${VERSION}.md" "VERIFICATION_${VERSION}.md" "COMPATIBILITY_${VERSION}.md" Makefile .github/workflows/ci.yml scripts/build.sh scripts/install.sh scripts/package.sh scripts/package_test.sh scripts/release_gate.sh tests/target_format_matrix.py tests/native_linker_driver_contract.py tests/native_host_smoke.py tests/c_differential.py tests/cross_differential.py tests/semantic_conversions.py tests/standard_modes.py tests/release_hardening.py tests/parser_fuzz.py tests/determinism.py tests/examples_smoke.py tests/optimizer_pipeline.py tests/backend_source_contract.py tests/backend_encoding_model.py tests/backend_codegen.py tests/scope_index.py tests/elf_tooling.py tests/scalability.py tests/source_integrity.py tests/optimizer_policy_contract.py tests/fast_fail.py tests/posix_headers.py tests/posix_abi_contract.py tests/optimizer_latency.py bench/compare.py src/rcc_backend_preflight.pas src/rcc_name_index.pas src/rcc_ast_inline.pas src/rcc_ast_reachability.pas src/rcc_native_linker.pas src/rcc_macho.pas src/rcc_object_writer.pas include/rcc/capabilities.h; do
