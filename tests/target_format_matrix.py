...
def main(argv: list[str]) -> int:
    ...
    # Apple arm64 passes every unnamed argument on the stack in consecutive
    # 8-byte slots.  This call has two integer varargs, so require a 16-byte
    # outgoing area, stores whose destinations are sp and sp + 8, and the
    # matching post-call cleanup.  Check the ABI-bearing instructions rather
    # than tying this test to one particular staging/copy sequence.
    apple_variadic_patterns = (
        struct.pack("<I", 0xD10043FF),  # sub sp, sp, #16
        struct.pack("<I", 0x910003E0),  # mov x0, sp
        struct.pack("<I", 0x910023E0),  # add x0, sp, #8
        struct.pack("<II", 0x94000000, 0x910043FF),  # bl; add sp, sp, #16
    )
    if any(pattern not in arm_variadic_text for pattern in apple_variadic_patterns):
        raise SystemExit(
            f"{arm_variadic_path}: Darwin arm64 variadic arguments are not "
            "stored in consecutive 8-byte stack slots"
        )
